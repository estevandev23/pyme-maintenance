...
- [x] 1.1 Ejecutar `npx prisma generate` y confirmar que los tipos del cliente
...
- [x] 1.2 Registrar el punto de partida ejecutando `npm run lint` y guardando el
...
- [x] 2.1 Tipar `andFilters: any[]` como `Prisma.EquipoWhereInput[]` en
...
- [x] 2.2 Tipar `andFilters: any[]` como `Prisma.MantenimientoWhereInput[]` en
...
- [x] 2.3 Cambiar `let whereClause: any` por `const whereClause:
...
- [x] 2.4 Tipar los dos `where: any` de `src/app/api/usuarios/route.ts` (líneas
...
- [x] 2.5 Tipar `where: any` como `Prisma.SolicitudServicioWhereInput` en
      `src/app/api/solicitudes/route.ts:27`. Verificar con lint y comprobar que
      el listado de solicitudes no cambia.
- [x] 2.6 Tipar `tx: any` como `Prisma.TransactionClient` en la función
...
- [x] 2.7 Tipar los dos `updateData: any` de
...
- [x] 3.1 Tipar `icon: any` como `LucideIcon` en `menuItems`
...
- [x] 3.2 Tipar `icon: any` como `LucideIcon` en `estadoConfig`
...
- [x] 4.2 Tipar `onSubmit(data: any)` y `handleSubmit(data: any)` en
...
- [x] 4.3 Tipar `handleUpdate(data: any)` en
...
- [x] 5.1 Declarar `fechaProgramada` y `fechaRealizada` como `string | Date` en
...
- [x] 5.2 Eliminar los dos `as any` de
...
- [x] 6.1 Sustituir los `any[]` de `exportEquiposToPDF`,
...
- [x] 6.2 Comprobar que `src/__tests__/lib/pdf-export.test.ts` sigue pasando sin
...
- [x] 7.1 Ejecutar `npm run lint` y verificar que termina sin errores y con
      código de salida cero.
- [x] 7.2 Verificar que la configuración de ESLint no rebajó ninguna regla y que
      no se añadió ningún comentario `eslint-disable` en los archivos saneados.
- [x] 7.3 Ejecutar `npm test` y verificar que la suite sigue pasando completa.
- [x] 7.4 Registrar aparte cualquier incoherencia real que haya destapado el
