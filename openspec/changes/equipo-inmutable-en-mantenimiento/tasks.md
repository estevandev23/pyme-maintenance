...
- [x] 1.1 Añadir a `src/lib/` una función pura que reciba el `equipoId` enviado y
...
- [x] 1.2 Añadir las pruebas de esa función en `src/__tests__/lib/`, cubriendo
      los cuatro casos de la tarea 1.1. Verificar con `npm test` que pasan.

## 2. Rechazo en el endpoint

- [x] 2.1 En `PUT /api/mantenimientos/[id]`, aplicar la comprobación de la tarea
...
- [x] 2.2 Redactar el mensaje de rechazo de modo que explique que el equipo se
...
- [x] 2.3 Comprobar que el mantenimiento no queda modificado tras un rechazo:
...
- [x] 3.1 Verificar desde el formulario que guardar una edición normal —que
      reenvía el `equipoId` actual— sigue funcionando y guarda los demás campos.
- [x] 3.2 Verificar que una actualización que omite `equipoId` se acepta y el
      mantenimiento conserva su equipo.
- [x] 3.3 Verificar que la reasignación de técnico introducida por
      `asignacion-automatica-tecnicos` sigue funcionando junto con esta
      comprobación, incluida su entrada en el historial.
- [x] 3.4 Verificar que la rama de TECNICO del endpoint no se ve afectada: usa
      `cambiarEstadoSchema`, que no acepta `equipoId`, y debe seguir cambiando
      estado y observaciones igual que antes.

## 4. Verificación integral

- [x] 4.1 Ejecutar `npm test` y verificar que la suite pasa completa.
- [x] 4.2 Ejecutar `npm run lint` y verificar que este cambio no añade errores
